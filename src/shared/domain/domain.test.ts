...
describe('Meeting', () => {
  ...
  it('defaults titleAutoGenerated to false when absent', () => {
    ...
    expect(meeting.titleAutoGenerated).toBe(false)
  })

  it('accepts titleAutoGenerated true (quick-start placeholder title)', () => {
    const meeting = MeetingSchema.parse({
      id: 'meeting-auto-2',
      state: 'live',
      createdAt: new Date().toISOString(),
    ...
    expect(meeting.titleAutoGenerated).toBe(true)
  })
