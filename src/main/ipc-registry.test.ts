...
// ---------------------------------------------------------------------------
// meeting:create persists the row (so the Draft title reaches the DB)
// ---------------------------------------------------------------------------

describe('IPC registry — meeting:create persistence', () => {
  const mockSettingsStore = {
    current: { asrProvider: 'deepgram', extractionProvider: 'anthropic', primaryLanguage: 'nl' },
    save: async () => {
      // no-op
  ...
  it('persists the created meeting with its title and titleAutoGenerated flag', async () => {
    ...
    const stored = mRepo.findById(created.id)
    expect(stored?.title).toBe('Mijn overleg')
    expect(stored?.titleAutoGenerated).toBe(true)
    expect(stored?.state).toBe('draft')
  })
})

// ---------------------------------------------------------------------------
// agendaItem/participant add + remove persistence (audit C1)
//
// The Draft screen prepares a meeting's agenda + participants. These handlers
// must write them to the DB so rolling-extraction routing sees the agenda,
// owner assignment has a participant list, and meeting:load restores them after
// a restart. Before the fix the handlers fabricated an id and persisted nothing.
// ---------------------------------------------------------------------------

describe('IPC registry — agenda/participant persistence (audit C1)', () => {
  const mockSettingsStore = {
    current: { asrProvider: 'deepgram', extractionProvider: 'anthropic', primaryLanguage: 'nl' },
    save: async () => {
      // no-op
  ...
  it('agendaItem:add persists the item so listByMeeting returns it', async () => {
    const db = makeDb()
    ...
    const stored = aiRepo.listByMeeting('mtg-1')
    expect(stored).toHaveLength(1)
    expect(stored[0]?.id).toBe(resp.id)
    expect(stored[0]?.title).toBe('Begroting')
    expect(stored[0]?.topic).toBe('Q3')
  })

  it('participant:add persists the participant so listByMeeting returns it', async () => {
    const db = makeDb()
    ...
    const stored = pRepo.listByMeeting('mtg-1')
    expect(stored).toHaveLength(1)
    expect(stored[0]?.id).toBe(resp.id)
    expect(stored[0]?.name).toBe('Alice')
  })

  it('agendaItem:remove deletes the persisted item', async () => {
    const db = makeDb()
    const aiRepo = agendaItemRepo(db)
    const registry = createIpcRegistry({
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-explicit-any
    ...
    await registry.dispatch('agendaItem:remove', { agendaItemId: resp.id })

    expect(aiRepo.listByMeeting('mtg-1')).toHaveLength(0)
  })

  it('participant:remove deletes the persisted participant', async () => {
    const db = makeDb()
    const pRepo = participantRepo(db)
    const registry = createIpcRegistry({
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-explicit-any
    ...
    await registry.dispatch('participant:remove', { participantId: resp.id })

    expect(pRepo.listByMeeting('mtg-1')).toHaveLength(0)
  })
})

// ---------------------------------------------------------------------------
// agendaItem:confirm and agendaItem:editAndConfirm (ADR 0029)
// ---------------------------------------------------------------------------

describe('IPC registry — agenda grooming (ADR 0029)', () => {
  const mockSettingsStore = {
    current: { asrProvider: 'deepgram', extractionProvider: 'anthropic', primaryLanguage: 'nl' },
    save: async () => {
      // no-op
  ...
  it('confirms a Proposed agenda item (state → confirmed)', async () => {
    const db = makeDb()
    const aiRepo = agendaItemRepo(db)
    aiRepo.insert({ id: 'ai-1', title: 'Begroting', topic: 'Q3', state: 'proposed' }, 'mtg-1')
    ...
    expect(aiRepo.findById('ai-1')?.state).toBe('confirmed')
  })

  it('edits and confirms a Proposed agenda item', async () => {
    const db = makeDb()
    const aiRepo = agendaItemRepo(db)
    aiRepo.insert({ id: 'ai-1', title: 'Oud', topic: 'oud', state: 'proposed' }, 'mtg-1')
    ...
    const item = aiRepo.findById('ai-1')
    expect(item?.title).toBe('Nieuw')
    expect(item?.state).toBe('confirmed')
  })

  it('rejects confirm for an unknown agenda item', async () => {
    const db = makeDb()
    const registry = createIpcRegistry({
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-explicit-any
      settingsStore: mockSettingsStore as any,
    ...
    await expect(
      registry.dispatch('agendaItem:confirm', { agendaItemId: 'nope' }),
    ).rejects.toThrow()
  })

  it('pauses and resumes a meeting via the injected handlers', async () => {
    const paused = {
      id: 'mtg-1',
      title: 'Test',
      state: 'live' as const,
    ...
    const resumeRes = (await registry.dispatch('meeting:resume', { meetingId: 'mtg-1' })) as {
      paused: boolean
    }
    expect(resumeRes.paused).toBe(false)
  })

  it('throws when the pause handler is absent', async () => {
    const registry = createIpcRegistry({
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-explicit-any
      settingsStore: mockSettingsStore as any,
    })
    await expect(registry.dispatch('meeting:pause', { meetingId: 'mtg-1' })).rejects.toThrow(
      'meeting:pause is not available',
    )
  })
})
