...
export class LiveSessionController {
  ...
  private _buildRuntime(meetingId: string): LiveExtractionRuntime {
    // Ensure the meeting row exists and is Live before spans reference it
    // (transcriptSpanRepo.insert has a foreign key on meetings). meeting:create
    // now persists the row with the real Draft title, so when it exists we only
    // transition it to Live — never overwrite the title. The "Active Meeting"
    // insert is a last-resort fallback for a missing row (no title is known here).
    const existing = this._meetingRepo.findById(meetingId)
    ...
    if (existing === null) {
      ...
      // Draft → Live through the single enforcer (sets startedAt, guards state).
      this._meetingLifecycle.startMeeting(meetingId)
    } else if (existing.state !== 'live') {
      // Defensive: an unexpected non-draft, non-live row (e.g. a re-entered
