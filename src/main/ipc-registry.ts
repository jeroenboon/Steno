...
export interface IpcRegistryDependencies {
  ...
  /**
   * Meeting repo. When present, meeting:create persists the new meeting row so
   * the Draft title (and the quick-start titleAutoGenerated flag) reach the DB
   * and are not lost to the LiveSessionController's "Active Meeting" fallback.
   */
...
function makeHandleMeetingCreate(deps: IpcRegistryDependencies) {
  return function handleMeetingCreate(raw: unknown): MeetingCreateResponse {
    ...
    deps.meetingRepo?.insert(meeting)
