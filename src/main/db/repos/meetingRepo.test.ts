...
describe('meetingRepo', () => {
  ...
  it('round-trips the titleAutoGenerated flag', () => {
    const repo = meetingRepo(db)
    repo.insert({ ...sample, id: 'mtg-auto', titleAutoGenerated: true })
    expect(repo.findById('mtg-auto')?.titleAutoGenerated).toBe(true)
  })

  it('defaults titleAutoGenerated to false for existing rows', () => {
    const repo = meetingRepo(db)
    repo.insert(sample)
    expect(repo.findById('mtg-1')?.titleAutoGenerated).toBe(false)
  })

  it('persists a titleAutoGenerated change through update', () => {
    const repo = meetingRepo(db)
    repo.insert({ ...sample, id: 'mtg-upd', titleAutoGenerated: true })
    repo.update({ ...sample, id: 'mtg-upd', title: 'Echte titel', titleAutoGenerated: false })
    const found = repo.findById('mtg-upd')
    expect(found?.title).toBe('Echte titel')
    expect(found?.titleAutoGenerated).toBe(false)
  })
