...
- Shared substrate keeps adapters thin: **`RealtimeSpanStream`** owns the realtime transport (queue/iterator, reconnect+backoff, decode) behind a `RealtimeAsrWire` seam for the streaming ASR adapters (ADR 0032); **`ExtractionEngine`** (`extractionEngine.ts`) owns the whole extraction contract (prompt body, per-item coercion, retry-degrade, devlog, inferContext) behind an `ExtractionWire` seam. All three families are thin over it: the OpenAI-compatible/Azure adapters via `OpenAiJsonWire` (fetch + json_object) and Anthropic via `AnthropicToolWire` (SDK forced tool use). Only the mechanism sentence and prompt caching stay per-wire (ADR 0034).
...
4. **Item lifecycle** — `ItemLifecycleService` enforces Proposed/Confirmed rules: the agent only ever creates/revises/retracts **Proposed** items; **Confirmed** items change only via explicit user edit. It exposes an `onItemsChanged(meetingId)` seam fired after every mutation; both the runtime (agent path) and `index.ts` (note-taker IPC path) wire it to `sendItemsChanged`, which pushes the **authoritative full item set for the meeting** (both states). Main is the single source of truth — the renderer reconciles wholesale, never re-deriving transitions (ADR 0033).
5. **MeetingContext** — a `MeetingContextOwner` owns the live context: `current()` (final pass + nudges), `routingContext()` (Confirmed-agenda-only for rolling turns, ADR 0029), `enrich()` (after final-pass inference).
