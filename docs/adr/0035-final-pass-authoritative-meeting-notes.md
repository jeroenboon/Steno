...
## Update — the final notes weren't reaching Review (2026-07-04)

Field test: a meeting whose final pass succeeded (devlog showed `isFinalPass, decisions 1/1, actions 2/2`) still rendered an **empty** Review — only the Off-agenda bucket, "no items", no summary. Three separate gaps, each of which independently lost notes:

1. **The agenda was never pushed to the renderer on end.** The final pass routes its items (and per-agenda summaries) onto the agenda it inferred/enriched, but the live `agenda:changed` stream stops when the meeting ends, and the Review transition reads the agenda from the store (no reload). So items routed onto agenda items the store didn't have landed under non-existent groups and silently vanished. Fix: `endMeeting` now emits `agenda:changed` with the full repo agenda right after the final pass.
2. **An empty final response wiped the live proposals.** The `retractAllProposed` from the first commit ran unconditionally; a degraded/empty final pass therefore erased everything. Fix: only supersede when the final response actually has ≥1 decision/action (a real replacement).
3. **Orphan items had no visible home.** Belt-and-suspenders: `ReviewScreen` and `toMarkdown` now surface any item whose `agendaItemId` isn't among the known agenda items under Off-agenda, so a routing/agenda-sync mismatch can never hide an item again.

(Separately, `gpt-4o-mini` sometimes returns **no** `discussionSummaries` at all on the final pass; that is a provider-prompt-adherence issue, not a plumbing one, and is tracked outside this ADR.)
