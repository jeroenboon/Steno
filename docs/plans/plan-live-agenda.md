...
### Step 3.3 — Final pass infers context + replaces auto-title for live meetings ✅ done **Slice 3 complete.**

> Note: runtime-end inference is gated on `meeting.source === 'live'` + an empty
> context agenda (the import path infers itself; re-inferring would double up).
> The runtime gained optional agendaItem/participant/meeting repos (wired from
> LiveSessionController + index.ts); absent ⇒ inference is skipped. Inferred
> agenda items are persisted Proposed and the in-memory context is enriched so
> the final pass routes into them. Title is replaced (and the flag cleared) only
> when `titleAutoGenerated` is set.
