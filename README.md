...
- **Real-time extraction**: Proposed Decisions and Actions appear as the meeting progresses, driven by a rolling extraction loop. Extraction runs on Anthropic, any OpenAI-compatible endpoint (OpenAI, Mistral, or a custom endpoint), Azure OpenAI, or a **local** LLM you run yourself (LM Studio, Ollama, llama.cpp) so notes never leave the device. See [docs/local-extraction.md](docs/local-extraction.md).
